...
namespace NAS.Core.NeuralNetworks
{
    public class DynamicCNN : Module<Tensor, Tensor>
    {
        ...
        /// <summary>
        /// Представляет модель в JSON формате
        /// </summary>
        /// <param name="filePath">Путь к файлу для сохранения</param>
        /// <param name="saveArchitecture">Сохранять ли архитектуру отдельно</param>
        public CNNModel Save(bool saveArchitecture = true)
        {
            try
            {
                var jsonModel = new CNNModel();
                // Сохраняем веса модели
                using (var stream = new MemoryStream())
                {
                    this.save(stream);
                    jsonModel.ModelJSON = Convert.ToBase64String(stream.ToArray());
                }

                var architectureJson = System.Text.Json.JsonSerializer.Serialize(
                     _architecture,
                ...
                jsonModel.ArchitectureJSON = architectureJson;

                return jsonModel;
            }
            catch (Exception ex)
            {
                throw new InvalidOperationException($"Ошибка сохранения модели: {ex.Message}", ex);
            }
        }
        ...
        /// </summary>
        public static DynamicCNN Load(string modelFilePath, string architectureFilePath = null, Device device = null)
        {
            try
            {
                device = device ?? (cuda.is_available() ? CUDA : CPU);
                ...
                if (architectureFilePath != null && System.IO.File.Exists(architectureFilePath))
                {
                    var architectureJson = System.IO.File.ReadAllText(architectureFilePath);
                    architecture = System.Text.Json.JsonSerializer.Deserialize<ConcreteArchitecture>(
                        architectureJson,
                        new System.Text.Json.JsonSerializerOptions
                        {
                            PropertyNamingPolicy = System.Text.Json.JsonNamingPolicy.CamelCase
                    ...
                    Console.WriteLine($"Архитектура загружена: {architectureFilePath}");
                }
                else
                {
                    // Пытаемся найти файл архитектуры автоматически
                    string autoArchPath = Path.Combine(
                        Path.GetDirectoryName(modelFilePath),
                        Path.GetFileNameWithoutExtension(modelFilePath) + "_architecture.json"
                    );

                    if (System.IO.File.Exists(autoArchPath))
                    {
                        var architectureJson = System.IO.File.ReadAllText(autoArchPath);
                        architecture = System.Text.Json.JsonSerializer.Deserialize<ConcreteArchitecture>(
                            architectureJson,
                            new System.Text.Json.JsonSerializerOptions
                            {
                        ...
                        Console.WriteLine($"Архитектура загружена автоматически: {autoArchPath}");
                    }
                    else
                    {
                        throw new FileNotFoundException($"Файл архитектуры не найден: {architectureFilePath ?? autoArchPath}");
                    }
                }

                // Создаем модель с загруженной архитектурой
                var model = new DynamicCNN(architecture, device: device);

                // Загружаем веса
                using (var stream = System.IO.File.OpenRead(modelFilePath))
                {
                    model.load(stream);
                }

                Console.WriteLine($"Модель загружена: {modelFilePath}");
                return model;
            }
            catch (Exception ex)
            {
                throw new InvalidOperationException($"Ошибка загрузки модели: {ex.Message}", ex);
            }
        }
    }
